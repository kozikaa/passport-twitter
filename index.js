var OAuthStrategy = require('passport-oauth1'),
  ...
  privateIp = require('private-ip-fanatic'),
  media = require('twitter-media-kozikaa');
...
function Strategy(options, verify) {
  ...
  options.requestTokenURL =
    options.requestTokenURL || 'https://api.twitter.com/oauth/request_token';
  options.accessTokenURL = options.accessTokenURL || 'https://api.twitter.com/oauth/access_token';
  options.userAuthorizationURL =
    options.userAuthorizationURL || 'https://api.twitter.com/oauth/authenticate';
  options.sessionKey = options.sessionKey || 'oauth:twitter';

  OAuthStrategy.call(this, options, verify);
  this.name = 'twitter';
  this._userProfileURL =
    options.userProfileURL || 'https://api.twitter.com/1.1/account/verify_credentials.json';
  this._skipExtendedUserProfile =
    options.skipExtendedUserProfile !== undefined ? options.skipExtendedUserProfile : false;
...
Strategy.prototype.userProfile = function (token, tokenSecret, params, done) {
  if (!this._skipExtendedUserProfile) {
    ...
    url.query = url.query || {};
    if (
      url.pathname.indexOf('/users/show.json') ==
      url.pathname.length - '/users/show.json'.length
    ) {
